...
import DUpdateMachinePopup from "../DUpdateMachinePopup";
...
const DeduruOya = () => {
  ...
  useEffect(() => {
    ...
    const fetchDailyData = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/deduruoya-data`, {
          headers: {
            "ngrok-skip-browser-warning": "true",
          },
        });
    ...
    const fetchImages = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/deduru-daily-images`,
          {
            headers: {
              "ngrok-skip-browser-warning": "true",
            },
        ...
        setSliderImages(response.data.slice(0, 5));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_BASE_URL}/deduruoya-data`, formData);
      setShowForm(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await axios.delete(`${API_BASE_URL}/deduruoya-data/${id}`);
